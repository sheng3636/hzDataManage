<template>
  <div class="appContation">
    ...
    <ul class="moudleWrap">
      <li @click="moudleNameClick('交通经济')" :class="{active:queryParams.moduleName === '交通经济'}">交通经济</li>
      <li @click="moudleNameClick('交通设施')" :class="{active:queryParams.moduleName === '交通设施'}">交通设施</li>
      <li @click="moudleNameClick('运输服务')" :class="{active:queryParams.moduleName === '运输服务'}">运输服务</li>
    </ul>
        ...
        <el-table ref="titlesTable" :data="titlesList" border fit highlight-current-row>
          ...
          <el-table-column width="180" align="center" fixed="right" label="操作">
            <template slot-scope="scope">
              <span class="optionBtn optionBtn1" @click="jumpUrl('TrafficDetail',scope.row)" v-permission="['dataMangement/detail']">查看详情</span>
            </template>
          </el-table-column>
        </el-table>
